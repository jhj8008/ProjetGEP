...
class ActivitésController extends Controller
{
    ...
    {   
        $articles = \App\Article::select("*")->where('type', 'like', 'activité')->get();
        $max_pages = round(count($articles) / 4);
        return view('activités', compact('max_pages'));
    }
    ...
    public function getAllArticles(Request $request, $type){
        ...
        if(isset($request->all()['last_article'])){
            $articles = \App\Article::distinct()->where('type', 'like', 'activité')->where('id', '>', $request->all()['last_article'])->orderBy('created_at')->paginate(4);
        }else {
            $articles = \App\Article::distinct()->where('type', 'like', 'activité')->orderBy('created_at')->paginate(4);
        }
