...
class GestionFichePersonnelleController extends Controller
{
    ...
    public function index(){
        $personnels = Employe::select("*")->where('personnel', '=','1')->get();
        $fiches = array();
        foreach($personnels as $p){
            array_push($fiches, $p->fiche_personnelle);
        }
        return view('Auth\employe\admin\fiches_personnelles', compact('fiches'));
    }

    public function getFichePersonnelle($fiche_id){
        $f_p = \App\Fiche_personnelle::find($fiche_id);
        return view('Auth\employe\admin\fiche_personnelle', compact('f_p'));
    }

    public function getFormModifierFiche($fiche_id){
        $fiche = \App\Fiche_personnelle::find($fiche_id);
        return view('Auth\employe\admin\form_modifier_fiche', compact('fiche'));
    }
    ...
    public function modifierFiche(Request $request, $fiche_id){
        ...
        return redirect()->route('admins.gestion_fiches_personnelles')->with('success', 'Fiche personnelle modifiée avec succés !');
    }
