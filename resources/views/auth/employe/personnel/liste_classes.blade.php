                ...
                <tbody class="text-body">
                    @foreach($classes as $classe)
                        <tr>
                            ...
                            <td class="align-middle">{{ count($classe->elèves) }}</td>
                            <td>
                                @if(count($classe->elèves) == 0)
                                    <button type="button" class="btn btn-secondary" disabled>{{ __('Télécharger') }}</button>
                                @else
                                    <a href="{{ route('personnels.print_liste_classe', ['id' => $classe->id]) }}" class="btn btn-outline-secondary download-pdf" title="Télécharger pdf">{{ __('Télécharger') }}</a>
                                @endif
                            </td>
                        </tr>
                    @endforeach
